...
<div class="container">
	<div class="row">
		...
		<div class="col-md-12 order-md-1">
			...
			<form action="@if(isset($parent_ids)) {{route('admin.catagory.update', $catagory)}} @else {{route('admin.catagory.store')}} @endif" method="post" accept-charset="utf-8">
	@csrf
	@if(isset($parent_ids))
		@method('PUT')
	@endif
				...
				<div class="row">
					<div class="col-md-12  col-lg-12 mb-3">
						...
						<input type="title" name="title" class="form-control" id="txturl" placeholder="" value="{{@$catagory->title}}" required>
						<p class="small" >{{config('app.url')}}- <span id="slug_url">{{@$catagory->slug}}</span></p>
				...
				<div class="row">
					<div class="col-md-12  col-lg-12 mb-3">
						...
						<select class=" form-control js-example-basic-multiple" id="stylo" name="parent_id[]" multiple="multiple">					
							...
							<option value="{{$cat->id}}" @if(!is_null($parent_ids) && in_array($cat->id, $parent_ids)) {{'selected'}} @endif> {{$cat->title}}</option>}
							option
							@endforeach
							@endif
							option
